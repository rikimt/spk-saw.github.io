	...
	function simpan_pengguna(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $nama=$this->input->post('xnamaPengguna');
							...
							$level=$this->input->post('xlevel');
     						if ($password <> $konfirm_password) {
     							echo $this->session->set_flashdata('msg','error');
	               				...
	               				$this->m_pengguna->simpan_pengguna($nama,$username,$password,$level,$gambar);
	            	...
	            	if ($password <> $konfirm_password) {
     					echo $this->session->set_flashdata('msg','error');
	               		...
	               		$this->m_pengguna->simpan_pengguna_tanpa_gambar($nama,$username,$password,$level);
	                    echo $this->session->set_flashdata('msg','success');
	               		redirect('pengguna');
	               	}
	            } 
                            ...
                            if (empty($password) && empty($konfirm_password)) {
	                    		...
	                    		echo $this->session->set_flashdata('msg','info');
     							...
     							echo $this->session->set_flashdata('msg','error');
	               				...
	               				$this->m_pengguna->update_pengguna($kode,$nama,$username,$password,$level,$gambar);
	                    		echo $this->session->set_flashdata('msg','info');
	               				redirect('pengguna');
	               			}
	            	...
	            	if (empty($password) && empty($konfirm_password)) {
	                    ...
	                    echo $this->session->set_flashdata('msg','info');
     					...
     					echo $this->session->set_flashdata('msg','error');
	               		...
	               		$this->m_pengguna->update_pengguna_tanpa_gambar($kode,$nama,$username,$password,$level);
	                    echo $this->session->set_flashdata('msg','warning');
	               		redirect('pengguna');
	               	}
	            } 
